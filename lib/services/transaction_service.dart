...
class TransactionService {
  ...
  TransactionService({
    ...
    // Inicializar todos los servicios dentro del cuerpo del constructor
  ...
  // Modificar la función _savePendingOperation
  ...
  Future<String> addTransaction(FinancialTransaction transaction) async {
    ...
    try {
      if (isConnected) {
        ...
        // Actualizar el balance local de la cuenta
  ...
  // Método para actualizar el balance local de la cuenta
  ...
  Future<void> moveToTrash(
    ...
    try {
      print('TransactionService: Moviendo a papelera: $transactionId');

      if (isConnected) {
        print('Modo online: Intentando mover a papelera en Firestore');
        final transactionRef = _firestore
            .collection('transactions')
            .doc(transactionId);
